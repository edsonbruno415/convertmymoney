...
test('getCotacao', ()=>{
    ...
    const getDataAtual = jest.fn()
    getDataAtual.mockReturnValue('01-01-2019')
    ...
    api.pure
        .getCotacao({getUrl, getCotacaoAPI, getDataAtual, extractCotacao})()
        .then( res => {
            expect(res).toBe(3.9)
        }).catch(err =>{
            expect(res).toBe('')
